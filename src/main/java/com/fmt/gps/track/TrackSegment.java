...
import com.fmt.gps.geocode.GeoCodeResult;
import com.fmt.gps.geocode.GeoCoder;

@JsonAutoDetect
public class TrackSegment {
	/** Number of points to skip for analysis. **/
	public static int STEP= 6;
	/** margin Seconds used to determine is speed change occurred. **/
	public static final int SECONDS_MARGIN= 3;

	/** Speed in Store. **/
	public static double STORE_SPEED_FPS= 2.0;
	/** Speed walking. **/
	public static double WALK_SPEED_FPS= 3.0;
	/** Speed running. **/
	public static double RUN_SPEED_FPS= 5.0;
	/** Speed bicycling. **/
	public static double BIKE_SPEED_FPS= 9.0; //11.0; //9.39
	/** speed in car in city. **/
	public static double VEHICLE_CITY_SPEED_FPS= 30.0;
	/** speed on highway. **/
	public static double HIGHWAY_SPEED_FPS= 45.0;
	/** speed flying. **/
	public static double AIR_SPEED_FPS= 150.0;
	/** Minimum Track Segment size when splitting up points and segments. **/
	public static int    MIN_SEGMENT_SIZE= 10;
	...
	public static double GPS_SPEED_FPS= 0.002724;
	public static final double MPH= 3447.136563877;
	public static  double TOLERANCE_PT_DIFF= 50.0;
	
	/** Difference in speeds tolerated between segments. **/
	public static double TOLERANCE_MERGE= 0.15;
	...
	public void setTextDescription(String newDesc) {
		this.textDescription= newDesc;
	}

	public String getTextDescription() {
		if(null == textDescription) {
			if(this.type.equals(caminarType.inside) || this.type.equals(caminarType.pauseIndoor)) {
				String hiatusDesc= (this.type.equals(caminarType.inside)) ? "stopped inside" : "stopped";
				textDescription= String.format("at %s I %s %s%s", Travelog.TIME.format(this.start), hiatusDesc, formatTime(this), Travelog.CR);
			} else {
				textDescription= String.format("at %s %s %s from %s to %s %s%s", Travelog.TIME.format(this.start), getMovementDesc(this.type), formatDistance(this.distance), getPlaceDesc(this.points.get(0)), getPlaceDesc(this.points.get(this.points.size()-1)), formatTime(this), Travelog.CR);
			}
		}
	...
	/**
	 * Describe position.
	 * @param trackPoint point to describe
	 * @return position description
	 **/
	private static String getPlaceDesc(TrackPoint trackPoint) {
		GeoCodeResult result= geoCoder.reverseGeoCode(trackPoint.getLat(), trackPoint.getLon());
		if(null == result) {
			return String.format("Lat: %.2f, Lon: %.2f", trackPoint.getLat(), trackPoint.getLon()); 
		} else {
			return result.formattedAddress; //streetNumber+ " "+ result.street+", "+ result.placename+", "+result.adminName1;
		}
	}
}
