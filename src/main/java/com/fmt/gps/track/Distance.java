...
public class Distance {
	...
	private static final SimpleDateFormat newgpxDate;

	static {
		gpxDate= new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
		gpxDate.setLenient(false);
		gpxDate.setTimeZone(TimeZone.getTimeZone("UTC"));
	}
	static {
		newgpxDate= new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
		newgpxDate.setLenient(false);
		newgpxDate.setTimeZone(TimeZone.getTimeZone("UTC"));
	}
	...
	public static SimpleDateFormat getNewGpxDateFormatter() {
		return (SimpleDateFormat)newgpxDate.clone();
	}
	...
	public static int getSecondsDiff(Date one, Date two) {
		return (null == two || one == null) ? 0 : Math.abs(((int)(two.getTime()- one.getTime()))/TrackSegment.LMILLIS);
	}
